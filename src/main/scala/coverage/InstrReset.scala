...
class InstrReset(mod: DefModule, mInfo: moduleInfo) {
  ...
  // private val regs = if (mInfo.assertReg != None || mInfo.regState != None) (mInfo.regs.toSeq :+ mInfo.assertReg.getOrElse() :+ mInfo.regState.get :+ mInfo.covSum.get).toSet else mInfo.regs
  private val regs = if (mInfo.assertReg != None && mInfo.regState != None) 
                        (mInfo.regs.toSeq :+ mInfo.assertReg.get :+ mInfo.regState.get :+ mInfo.covSum.get :+ mInfo.covMap.get).toSet 
                      else if (mInfo.assertReg != None)
                        (mInfo.regs.toSeq :+ mInfo.assertReg.get).toSet 
                      else if(mInfo.regState != None)
                        (mInfo.regs.toSeq :+ mInfo.regState.get :+ mInfo.covSum.get :+ mInfo.covMap.get).toSet 
                      else 
                        mInfo.regs
