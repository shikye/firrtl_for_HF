  ...
  */
  ...
  def getInstances: Set[DefInstance] = {
    for (inst <- defInstances) {
      if (inst.getClass.getSimpleName != "DefInstance"){
        println(inst.getClass.getSimpleName())
        throw new Exception(s"${inst.serialize} is not DefInstance class\n")
      }
        
    }
    defInstances.map(_.asInstanceOf[DefInstance]).toSet
  }
